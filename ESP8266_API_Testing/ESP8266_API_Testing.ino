...
*/
const char* yourSSID = "The LAN Before Time 2.4G";
...
void loop() {
  ...
  if ((WiFiMulti.run() == WL_CONNECTED)) {
    ...
    if (https.begin(*client, "https://ipapi.co/" + yourIP + "/json/")) {  // Sub in your IP here and comment above (set variable in variable decleration at the top)
  ...
  if (https.begin(*client, "http://api.ipify.org")) {  // GET the ESP IP address
      ...
      if (httpCode > 0) {
        ...
        // file found at server
        if (httpCode == HTTP_CODE_OK || httpCode == HTTP_CODE_MOVED_PERMANENTLY) {
          String myIP = https.getString(); // Set payload as server response
          
          Serial.println("The following is ESPs public IP address");
          Serial.println("------------------------------------------------");
          Serial.println(myIP);
        }
      }
